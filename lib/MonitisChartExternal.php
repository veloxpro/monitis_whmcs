...
class MonitisChartExternal extends MonitisChart {
	...
	function __construct($monitorID) {
		...
		$this->hchart->plotOptions->spline->marker->symbol = "circle";
		$this->hchart->plotOptions->spline->marker->states->hover->radius = 4;
	...
	function getData() {
		...
		foreach ($data as $location) {
			...
			foreach ($location['data'] as $point) {
				/*$o = new stdClass();
				$o->x = strtotime($point[0] . ' UTC')*1000;
				$o->y = $point[1];
				if ($o->y == 0) {
					$o->marker->fillColor = 'red';
					$o->marker->enabled = true;
				}
				$d[] = $o;*/
					
				$d[] = array(strtotime($point[0] . ' UTC')*1000, $point[1]);
			}
				
			$allLocations = $this->getExternalLocations();
			
			$this->addSeries($allLocations[$location['id']]['fullName'], $d);
			$series[$allLocations[$location['id']]['fullName']] = $d;
		}
